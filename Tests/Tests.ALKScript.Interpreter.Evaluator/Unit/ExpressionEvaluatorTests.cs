...
public class ExpressionEvaluatorTests
{
  ...
  [InlineData(1.5)]
  [InlineData("text")]
  public async Task Eval_Literal_ProducesTheMatchingValueType(object? literalValue)
  {
    var context = new FakeEvaluationContext();
    var value = await MakeEvaluator(context).Eval(Nodes.Literal(literalValue), new ScriptEnvironment());
  ...
  [Fact]
  public async Task Eval_Identifier_ResolvesAgainstTheEnvironment()
  {
    var context = new FakeEvaluationContext();
    var environment = new ScriptEnvironment();
    environment.Define("x", new IntValue(9));

    var value = await MakeEvaluator(context).Eval(Nodes.Ident("x"), environment);

    Assert.Equal(9L, Assert.IsType<IntValue>(value).Value);
  }

  [Fact]
  public async Task Eval_UndefinedIdentifier_ThrowsRuntimeException()
  {
    var context = new FakeEvaluationContext();

    var exception = await Assert.ThrowsAsync<RuntimeException>(() => MakeEvaluator(context).Eval(Nodes.Ident("missing"), new ScriptEnvironment()));

    Assert.Contains("Undefined name 'missing'", exception.Message);
  }

  [Fact]
  public async Task Eval_Grouping_DelegatesToTheInnerExpression()
  {
    var context = new FakeEvaluationContext();

    var value = await MakeEvaluator(context).Eval(new GroupingExpr(Nodes.Literal(3L)), new ScriptEnvironment());

    Assert.Equal(3L, Assert.IsType<IntValue>(value).Value);
  }

  [Fact]
  public async Task Eval_ArrayLiteral_EvaluatesEachElementInOrder()
  {
    var context = new FakeEvaluationContext();
    var elements = new List<Expr> { Nodes.Literal(1L), Nodes.Literal(2L), Nodes.Literal(3L) };

    var value = Assert.IsType<ArrayValue>(await MakeEvaluator(context).Eval(new ArrayLiteralExpr(elements), new ScriptEnvironment()));

    Assert.Equal(new long[] { 1, 2, 3 }, value.Items.ConvertAll(item => ((IntValue)item).Value));
  }

  [Fact]
  public async Task Eval_AssignmentToIdentifier_UpdatesTheBindingAndReturnsTheValue()
  {
    var context = new FakeEvaluationContext();
    var environment = new ScriptEnvironment();
    ...
    var value = await MakeEvaluator(context).Eval(assignment, environment);
  ...
  [Fact]
  public async Task Eval_AssignmentToUndefinedIdentifier_ThrowsRuntimeException()
  {
    var context = new FakeEvaluationContext();
    var assignment = new AssignmentExpr(Nodes.Ident("missing"), Nodes.Literal(1L));

    var exception = await Assert.ThrowsAsync<RuntimeException>(() => MakeEvaluator(context).Eval(assignment, new ScriptEnvironment()));
  ...
  public async Task Eval_LogicalOperators_ShortCircuitWithoutEvaluatingTheRightOperand(
    ALKScriptTokenType operatorType, string lexeme, bool leftValue, bool expectRightEvaluated, bool expectedResult)
  {
    ...
    var result = await MakeEvaluator(context).Eval(binary, environment);
  ...
  public async Task Eval_ArithmeticBinary_DelegatesToOperators(ALKScriptTokenType operatorType, string lexeme, long expected)
  {
    var context = new FakeEvaluationContext();
    var binary = new BinaryExpr(Nodes.Literal(1L), Nodes.Operator(operatorType, lexeme), Nodes.Literal(2L));

    var value = await MakeEvaluator(context).Eval(binary, new ScriptEnvironment());

    Assert.Equal(expected, Assert.IsType<IntValue>(value).Value);
  }

  [Fact]
  public async Task Eval_UnaryBang_NegatesTruthiness()
  {
    var context = new FakeEvaluationContext();
    var unary = new UnaryExpr(Nodes.Operator(ALKScriptTokenType.Bang, "!"), Nodes.Literal(true));

    var value = await MakeEvaluator(context).Eval(unary, new ScriptEnvironment());

    Assert.False(Assert.IsType<BoolValue>(value).Value);
  }

  [Fact]
  public async Task Eval_UnaryMinus_NegatesNumericOperand()
  {
    var context = new FakeEvaluationContext();
  ...
  [Fact]
  public async Task Eval_UnaryMinus_OnNonNumericOperand_ThrowsRuntimeException()
  {
    var context = new FakeEvaluationContext();
    var unary = new UnaryExpr(Nodes.Operator(ALKScriptTokenType.Minus, "-"), Nodes.Literal("text"));

    await Assert.ThrowsAsync<RuntimeException>(() => MakeEvaluator(context).Eval(unary, new ScriptEnvironment()));
  }

  [Fact]
  public async Task Eval_Call_EvaluatesCalleeAndArgumentsThenDelegatesToTheContext()
  {
    var context = new FakeEvaluationContext();
    ...
    var value = await MakeEvaluator(context).Eval(call, environment);
  ...
  [Fact]
  public async Task Eval_New_EvaluatesArgumentsThenDelegatesConstructionToTheContext()
  {
    ...
    var value = await MakeEvaluator(context).Eval(newExpr, environment);
  ...
  [Fact]
  public async Task Eval_NewWithNonClassTypeName_ThrowsRuntimeException()
  {
    var context = new FakeEvaluationContext();
    var environment = new ScriptEnvironment();
    ...
    var exception = await Assert.ThrowsAsync<RuntimeException>(() => MakeEvaluator(context).Eval(newExpr, environment));

    Assert.Contains("'NotAClass' is not a class", exception.Message);
  }

  [Fact]
  public async Task Eval_GetOnInstance_ReturnsFieldValue()
  {
    var classDecl = new ClassDecl(false, Nodes.Identifier("Foo"), System.Array.Empty<string>(), null, System.Array.Empty<TypeNode>(), System.Array.Empty<MemberDecl>());
    ...
    var context = new FakeEvaluationContext();
    var value = await MakeEvaluator(context).Eval(get, environment);

    Assert.Equal("Ada", Assert.IsType<StringValue>(value).Value);
  }

  [Fact]
  public async Task Eval_GetOfUndefinedProperty_ThrowsRuntimeException()
  {
    var classDecl = new ClassDecl(false, Nodes.Identifier("Foo"), System.Array.Empty<string>(), null, System.Array.Empty<TypeNode>(), System.Array.Empty<MemberDecl>());
    var instance = new InstanceValue(new ClassValue(classDecl, null, new ScriptEnvironment()));
    var environment = new ScriptEnvironment();
    ...
    var context = new FakeEvaluationContext();
    var exception = await Assert.ThrowsAsync<RuntimeException>(() => MakeEvaluator(context).Eval(get, environment));

    Assert.Contains("Undefined property 'missing'", exception.Message);
  }

  [Fact]
  public async Task Eval_Index_ReturnsTheElementAtThePosition()
  {
    var environment = new ScriptEnvironment();
    ...
    var context = new FakeEvaluationContext();
    var value = await MakeEvaluator(context).Eval(index, environment);

    Assert.Equal(20L, Assert.IsType<IntValue>(value).Value);
  }

  [Fact]
  public async Task Eval_IndexOutOfBounds_ThrowsRuntimeException()
  {
    var environment = new ScriptEnvironment();
    ...
    var context = new FakeEvaluationContext();
    var exception = await Assert.ThrowsAsync<RuntimeException>(() => MakeEvaluator(context).Eval(index, environment));

    Assert.Contains("out of bounds", exception.Message);
  }

  [Fact]
  public async Task Eval_AlreadyPendingSignal_ShortCircuitsToNull()
  {
    var context = new FakeEvaluationContext { Signal = Signal.Return(NullValue.Instance) };

    var value = await MakeEvaluator(context).Eval(Nodes.Literal(1L), new ScriptEnvironment());
