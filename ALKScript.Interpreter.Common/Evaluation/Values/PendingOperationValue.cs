...
namespace ALKScript.Interpreter.Common.Evaluation.Values
{
  ...
  ///
  /// Unlike <see cref="ThunkValue"/> — which is always already-settled, the
  /// shape natives that resolve synchronously produce (mirroring C#/JS
  /// eager-start) — this defers actually starting the underlying host-side
  /// effect until <see cref="Start"/> is first called: on `await` ("Suspend"),
  /// or at end-of-script for any operation nobody ever awaited ("Discard",
  /// decision #10). Calling the `native` function merely records the request;
  /// reporting "as seen from script" identically to <see cref="ThunkValue"/>
  /// (<see cref="TypeName"/> is the same `"thunk"`) is what lets `await` treat
  /// both uniformly.
  ///
  ...
  /// returns the same status and starts the host effect at most once, which is
  ...
  public sealed class PendingOperationValue : ALKScriptValue
  {
    ...
    /// at end-of-script.
    /// </summary>
    public bool HasStarted => _status != null || _replayed;
    ...
    /// its (possibly still-pending) status.
    /// </summary>
    public OperationStatus Start() => _status ??= _binder.Start(Operation);

    /// <summary>
    /// Re-checks this operation's status via <see cref="IAsyncOperationBinder.Poll"/>
    /// if it was last reported <see cref="OperationStatus.Pending"/> — a no-op
    /// if <see cref="Start"/> has not been called yet, or the operation has
    /// already settled. Called by the host's "Pump" (see <c>ProgramRun.Pump</c>).
    /// </summary>
    public OperationStatus Poll()
    {
      if (_status is OperationStatus.Pending)
      {
        _status = _binder.Poll(Operation);
      }

      return _status ?? OperationStatus.Pending.Instance;
    }

    /// <summary>
    /// Directly settles this operation's status without invoking the binder —
    /// used by "Phase B" structural Restore (docs/ASYNC_AWAIT_DESIGN.md
    /// Addendum 3, Step 14) to reconstruct a <see cref="PendingOperationValue"/>
    /// whose captured outcome was a fault (see <see cref="OperationStatus.Faulted"/>),
    /// a state <see cref="ThunkValue"/> can no longer represent.
    /// </summary>
    internal void MarkSettled(OperationStatus status) => _status = status;

    /// <summary>
    /// The status <see cref="Start"/> produced, if it has been called —
    /// without triggering <see cref="Start"/>'s side effect. Used by "Phase C"
    ...
    /// </summary>
    internal OperationStatus? Status => _status;
    ...
    public override string ToString() =>
      ...
      _status switch
      {
        null => "<task not started>",
        OperationStatus.Pending => "<task pending>",
        _ => "<task completed>",
      };
  }
}
